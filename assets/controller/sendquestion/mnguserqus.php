<?php 
	...
	$id = $_SESSION['uid'];
	$select_qusans  = "SELECT allquestions.id,allquestions.uid,allquestions.questions,allquestions.date,allquestions.code FROM `allquestions` WHERE allquestions.uid = '$id'";
	...
	if($exe_total->num_rows > 0){
		while ($data_qs = $exe_total->fetch_object()) {
			...
			$mydata   = array();
			$response = "failure";
			$message  = "Not yet asked any question";		
	}
